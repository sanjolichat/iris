...
def call_groq(product_label):
    if not API_KEY:
        return {
            "analysis": "No Groq API key configured. Set GROQ_API_KEY to enable live AI scans.",
            "threat": "UNKNOWN",
            "metric1_label": "AI Scan", "metric1_value": "OFF",
            "metric2_label": "Recyclable", "metric2_value": "—",
            "metric3_label": "Impact", "metric3_value": "—",
            "tags": ["NO API KEY"]
        }
...
def gaze_thread(estimator):
    ...
    if not cap.isOpened():
        print("[iris] ERROR: Could not open webcam (index 0).")
        return
    print("[iris] Webcam opened — gaze thread running.")
    frames = 0
    while True:
        ret, frame = cap.read()
        if not ret:
        ...
        if features is not None and not blink:
            try:
                ...
                if frames % 120 == 0:
                    print(f"[iris] gaze predict error: {e}")
    cap.release()
...
if __name__ == "__main__":
    ...
    if not API_KEY:
        # Only prompt if we have an interactive terminal; otherwise run keyless.
        if sys.stdin is not None and sys.stdin.isatty():
            try:
                ...
                API_KEY = ""
    if not API_KEY:
        print("[iris] WARNING: No GROQ_API_KEY set — running without AI scans.")
        print("[iris] Eye tracking + HUD still work; scans will show a notice.")
    else:
        print("[iris] Groq API key loaded.")
